...
public class CreateUserTypeUseCase {
    ...
    private final LoggerUserGateway loggerUserGateway;

    public CreateUserTypeUseCase(RoleGateway roleGateway, UserTypeGateway userTypeGateway, LoggerUserGateway loggerUserGateway) {
        ...
        Objects.requireNonNull(loggerUserGateway, "LoggerUserGateway cannot be null");

        this.roleGateway = roleGateway;
        this.userTypeGateway = userTypeGateway;
        this.loggerUserGateway = loggerUserGateway;
    }

    public UserType execute(UserTypeInput input) {
        Objects.requireNonNull(input, "UserTypeInput cannot be null.");

        if (!loggerUserGateway.hasRole(CREATE_USER_TYPE_ROLE))
